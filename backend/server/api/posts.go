...
func (app App) createPost(w http.ResponseWriter, r *http.Request) {
	...
	if !database.IsPostPrivacy(request.Privacy) {
		writeJSON(w, http.StatusBadRequest, map[string]any{
			"status":  false,
	...
	if err = database.ValidateSelectedIDs(request.Privacy, request.SelectedFollowerIDs); err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]any{
			"status":  false,
			"message": err.Error(),
		})
	...
	post, err := database.CreatePost(app.DB, userID, request)
	if errors.Is(err, database.ErrSelectedFollowersRequired) || errors.Is(err, database.ErrInvalidPostViewer) {
		writeJSON(w, http.StatusBadRequest, map[string]any{
			"status":  false,
			"message": err.Error(),
		})
...
func (app App) listPosts(w http.ResponseWriter, r *http.Request) {
	...
	posts, err := database.ListFeedPosts(app.DB, userID)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{
			"status":  false,
